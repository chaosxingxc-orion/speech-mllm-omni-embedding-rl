...
Last updated: 2026-06-26 (dated correction added 2026-07-11: MInDS-14 reproduction row marked
superseded, see the Tool / Intent table)
...
| MInDS-14 reproduction | 182 | raw-schema Acc@1 = 0.852 | policy Acc@1 = 0.984, delta +0.132 CI [0.082, 0.187] | mixed, mostly system-side | **SUPERSEDED 2026-07-11 (ticket #33)** — see `MInDS-14 v2 (support/eval-separated, factorial)` row below. Forensic audit INT-001/INT-007 (verified) found (a) the committed `_repro/minds14_toolintent_paired.json` was hand-assembled — its own script (`scripts/repro_minds14_toolintent.py`) only prints the CI to stdout and never writes that file — and its numbers do not even match this row: the committed JSON says raw-schema=0.857/delta=+0.126 CI[0.077,0.181], not this row's raw-schema=0.852/delta=+0.132 CI[0.082,0.187]; (b) the candidate cards were built from 3 exemplar transcripts/class drawn from the SAME 182 rows being evaluated (42/182 eval rows were themselves a card exemplar) — transductive, not zero-shot; (c) instruction wording, card style, and example-presence were changed simultaneously, so the reported gain cannot be attributed to any one factor. `scripts/repro_minds14_toolintent.py` and this JSON are kept FROZEN as the audited record; they are not deleted or edited. |
...
| MInDS-14 v2 (support/eval-separated, factorial) | 437 eval / 126 card-pool (of 563 total, en-US, seed `CARD_SEED=20260711`) | `naive` (raw instruction, basic label, no examples) Acc@1 = 0.7117 | `instruction_plus_cards` (task instruction + contrastive-boundary card, pooled over 3 disjoint card-pool draws) Acc@1 = 0.9733, delta vs naive +0.2616 CI [0.2212, 0.3059], significant | mostly system-side (cards factor dominates); the instruction factor alone is a significant REGRESSION | 2026-07-11 clean redo (ticket #33), replacing the superseded row above: rows split ONCE into a disjoint card pool and eval set (verified 0 overlap; MInDS-14 ships only a single HF "train" parquet per language, no separate split to prefer), so cards can never see an eval row. 2x2 factorial isolates the two confounded factors: `instruction_only` (the only arm allowed "zero-shot" — task instruction, no examples) reaches Acc@1 = 0.4668, a **significant regression** vs naive (delta −0.2449, CI [−0.2860, −0.2014]) — the audio instruction alone hurts. `cards_only` (raw instruction + example cards, no task instruction) reaches pooled Acc@1 = 0.9580 (delta +0.2464 vs naive, CI [0.2052, 0.2922]) — essentially all of the gain is the candidate-card factor. Adding the instruction on top of cards gives a small but significant further gain (`instruction_plus_cards` vs `cards_only`: delta +0.0153, CI [0.0061, 0.0259]) — a real but minor positive interaction only when cards are already present. Full per-arm/per-draw results, manifests (ids + sha256), and provenance (git SHA, model/embedder hashes, seeds) in `_repro/minds14_toolintent_v2.json`, written atomically by `scripts/repro_minds14_toolintent_v2.py` (the sole writer of that file). |

> **v2 验后 caveat（Opus 复核 ACCEPT-with-notes，2026-07-11）**：card 臂数字（0.958/0.973）非纯净泛化指标——MInDS-14 为脚本化语料，9 条 card 文本与 eval 转写完全相同（表面重叠贡献约 +3/107 命中，≈3%，不改变 card-factor-driven 结论）；zero-shot 回归 headline（−0.245）与该重叠完全正交。
...
| SLURP tool instruction gate | 500 | raw Acc@1 = 0.550; `tool_specific_intent` Acc@1 = 0.582 | changed-same-family gate positive in 5/5 seeds; mean locked delta +0.065, mean LCB +0.027, route rate 0.097, regression rate 0.008 | omni-side controller | Accepted training-free policy: use instruction only for same-family action-boundary refinement |
| SLURP formal selector with gates | 500 | locked raw Acc@1 = 0.620 | selector chooses `tool_specific_same_family_gate`; locked Acc@1 = 0.665, delta +0.045 CI [0.010, 0.080], fixes/regressions 11/2 | omni-side selector | First robust public tool-semantic accepted policy over frozen omni outputs |
| MInDS formal selector with gates | 180 | locked raw Acc@1 = 0.861 | raw fallback; materialized gates route 0 rows or reject harmful global instructions | omni-side selector negative | Selector correctly refuses to force SLURP-style gate onto a strong raw MInDS baseline |
| MInDS low-margin top-k verifier | 180 | raw Acc@1 = 0.883, R@3 = 0.972 | low-margin top-3 LLM verifier Acc@1 = 0.956, delta +0.072 CI [0.039, 0.111], route rate 0.350, fixes/regressions 13/0 | downstream controller over omni outputs | Strong system-level positive; not a pure instruction gain |
...
| CoVoST2 ar->en low-margin top-k verifier | 200 | raw Acc@1 = 0.775, R@3 = 0.915 | low-margin top-3 LLM verifier Acc@1 = 0.905, delta +0.130 CI [0.085, 0.175], route rate 0.340, fixes/regressions 26/0 | downstream controller over omni outputs | Strong translation verifier positive after instruction arms fail |
| CoVoST2 zh-CN->en low-margin top-k verifier | 200 | raw Acc@1 = 0.985, R@3 = 0.995 | low-margin top-3 LLM verifier Acc@1 = 0.995, delta +0.010 CI [0.000, 0.025], route rate 0.040, fixes/regressions 2/0 | saturated sanity | Positive but underpowered; do not headline |
...
| Jina omni-small SLURP / CoVoST2 instruction transfer | local checkpoint available | cross-model instruction selector check | SLURP raw/tool instruction both Acc@1 0.564; CoVoST2 ar raw/translation both 0.635; CoVoST2 zh raw/translation both 0.970; selectors fall back to raw | Current Nemotron instruction arms are no-ops on Jina under the correct media-path interface |
